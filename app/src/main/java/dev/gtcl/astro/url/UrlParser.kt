...
val GFYCAT_REGEX =
    "(?:(?:http[s]?://)?(?:\\w+\\.)?gfycat\\.com)/(?:gifs/detail/)?(\\w+).*".toRegex()
...
val IMGUR_ALBUM_REGEX =
    "(?:(?:http[s]?://)?(?:\\w+\\.)?imgur\\.com)/(?:a|gallery)/(\\w+)/?".toRegex()
val IMGUR_IMAGE_REGEX = "(?:(?:http[s]?://)?(?:\\w+\\.)?imgur\\.com)/(\\w+)/?".toRegex()
val SUBREDDIT_REGEX =
    "(?:(?:http[s]?://)?(?:\\w+\\.)?reddit\\.com/|/)?r/([A-Za-z0-9_.]+)/?".toRegex()
val REDDIT_USER_REGEX =
    "(?:(?:http[s]?://)?(?:\\w+\\.)?reddit\\.com/|/)?(?:u|user)/([A-Za-z0-9_.]+)/?".toRegex()
val REDDIT_GALLERY_REGEX = "(?:http[s]?://)?\\w+\\.reddit\\.com/gallery/(\\w+)/?.*".toRegex()
val REDDIT_THREAD_REGEX =
    "(?:(?:http[s]?://)?(?:\\w+\\.)?reddit\\.com/|/)?(r/[A-Za-z0-9_.]+/comments/\\w+/\\w+/\\w+/?.*)".toRegex()
